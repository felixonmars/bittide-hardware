...
runProgramExpect act n expected = withSystemTempFile "ELF" $ \fp _ -> do
  ...
  let output = getPrintContents (BS.length expected) 0x70000000 $
  ...
  paths <- forM (L.zip binaryPaths expectFiles) $ \(binPath, expectPath) -> do
    ...
    if exists
      then
        ...
        hPutStrLn stderr $ "No binary file found for test program " <> takeBaseName binPath <> " (expected path " <> binPath <> ")"
        pure Nothing

  pure $ catMaybes paths

sourceDir, releaseBinDir, debugBinDir :: FilePath
sourceDir = "vex-riscv/vex-test-programs/src/bin"
...
debugCycles = 100_000_000 -- 100 million cycles
