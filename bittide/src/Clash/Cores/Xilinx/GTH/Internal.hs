...
gthCore
  ...
  !_drpclk_in
  !_gtrefclk0_in
 -- = deepErrorX "No Haskell model for gthCore"
...
-- doesn't work reliably yet with multiple channels
gthCoreN
  :: SNat chansUsed
  -> SNat quadsUsed
  -> Vec chansUsed String -- ^ channels
  -> Vec chansUsed String -- ^ ref clock paths

  -> "gtrefclk_in" ::: Clock refclk
  -> "drpclk_in" ::: Clock freerun

  -> "gthrxn_in" ::: Signal rxS (BitVector chansUsed)
  -> "gthrxp_in" ::: Signal rxS (BitVector chansUsed)
  ...
  -> "gtwiz_userdata_tx_in" ::: Signal txUser2 (BitVector (chansUsed*TX_DATA_WIDTH))
  -> "txctrl2_in" ::: Signal txUser2 (BitVector (chansUsed*(DivRU TX_DATA_WIDTH 8)))

  ->
   ( "gthtxn_out" ::: Signal txS (BitVector chansUsed)
   , "gthtxp_out" ::: Signal txS (BitVector chansUsed)

   , "gtwiz_userclk_tx_usrclk2_out" ::: Clock txUser2
   , "gtwiz_userclk_rx_usrclk2_out" ::: Clock rxUser2

   , "gtwiz_userdata_rx_out" ::: Signal rxUser2 (BitVector (chansUsed*RX_DATA_WIDTH))
...
{-# NOINLINE gthCoreN #-}
{-# ANN gthCoreN hasBlackBox #-}
{-# ANN gthCoreN (
   let primName = 'gthCoreN
       tfName = 'gthCoreNBBF
   in InlineYamlPrimitive [minBound..] [__i|
        BlackBoxHaskell:
            name: #{primName}
